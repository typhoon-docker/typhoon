...
export const importMocks = async () => {
  const [MockAdapter, { default: randomArray }, { default: logMock }] = await Promise.all([
  ...
  const [mockUser, mockProjects, mockContainers, mockBuildLogs, mockDockerFiles] = await Promise.all([
    import('./mock/user.json'),
    (await import('./mock/projects.json')).map(project => ({ ...project, owner: mockUser })),
    import('./mock/containers.json'),
    import('./mock/buildLogs.json'),
    import('./mock/dockerFiles.json'),
  ]);

  // getProjects
  mock.onGet('/projects').reply(logMock(200, mockProjects));

  // getAllProjects
  mock.onGet('/projects?all').reply(logMock(200, mockProjects));

  // postProject
  mock.onPost('/projects').reply(
    logMock(({ data }) => {
  ...
  // putProject
  mock.onPut(/\/projects\/\d+/).reply(
    logMock(({ data, url, baseURL }) => {
      const projectID = parseInt(url.substring(baseURL.length + 10), 10);
      const project = JSON.parse(data);
  ...
  // checkProject
  mock.onGet(/\/checkProject\?name=\w(\w|-)*/).reply(
    logMock(({ url, baseURL }) => {
...
2019-02-19T15:23:12.631459136Z 172.22.0.3 - - [19/Feb/2019:15:23:12 +0000] "GET /src.e3e4955e.css HTTP/1.1" 304 0 "https://typhoon.viarezo.fr/project/5c69f587a10f3c0001fb522e" "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_14_3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/72.0.3626.109 Safari/537.36" "138.195.247.76"`,
  ...
  // getBuildLogs
  mock.onGet(/\/docker\/buildLogs\/.*/).reply(logMock(200, mockBuildLogs));

  // getDockerFiles
  mock.onGet(/\/docker\/files\/.*/).reply(logMock(200, mockDockerFiles));
